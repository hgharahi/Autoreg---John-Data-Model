function [Testset] = Extract_Data_2( i, t_off_count, t_off_cutoff, PLV_smoothing_factor)
% This function extracts data and finds t_off for the 2nd control subject.
% This is an exception since all the other data could use the same cutoff
% value for all CPP cases, but here we needed to single out CPP 100 mmHg to
% find its t_off. 

load('data_loaded_full.mat');
...
%     Data.Flow = Data.Flow(end-tstep_from_end:end);
    ...
    while (t1 < Data.t(end) && flg == 0)
        ...
        if CPPcase(j) == 100
        count = sum(abs(Data.Flow((Data.t<(t1+1)) & (Data.t>t1)))<60);
        else
    ...
    figure;
    subplot(2,1,1);plot(Data.t, Data.AoP,'LineWidth',2);
    hold on;
    subplot(2,1,1);plot(Data.t, Data.CPP,'LineWidth',2);
