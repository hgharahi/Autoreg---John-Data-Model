function [Testset] = Extract_Data_HD( i, t_off_count, t_off_cutoff, PLV_smoothing_factor, t_after)
% clear;clc;close all
% i = 1
...
%     Data.Flow = Data.Flow(end-tstep_from_end:end);
    ...
    tstep_start = idx_t_off - ceil(Data.t_off/dt)+1; 
    tstep_end = min(idx_t_off + ceil(t_after/dt),length(Data.t)); %% sets the index of the last point that we want to consider: t_after seconds after clamping the circuit. 
